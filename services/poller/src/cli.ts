 ...
 *
 * Default mode connects to DATABASE_URL and upserts per contract §2.
 */

function usage(): never {
  console.error(`usage: pnpm poll <${SOURCES.join("|")}|all> [--dry-run] [--fixture[=path]]`);
  process.exit(2);
}

const args = process.argv.slice(2);
let target: Source | "all" | null = null;
let dryRun = false;
